...
const instructionsArray = craneInstructions.split("\n");
...
instructionsArray.forEach((instruction) => {
  ...
  // move boxes from `from` to `to`
  for (let index = 1; index <= numberToMove; index++) {
    const box = boxMap[from].pop();
    if (!box) {
      // this would indicate a problem with the loop/logic
...
const lastBoxInEachRow = Object.values(boxMap).map(
